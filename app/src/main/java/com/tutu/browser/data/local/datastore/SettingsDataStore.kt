package com.tutu.browser.data.local.datastore
...
class SettingsDataStore(private val context: Context) {
    ...
    companion object {
        ...
        // Background playback & Floating window (mutually exclusive)
        ...
        .map { prefs ->
            try {
                TutuSettings(
                    ...
                    backgroundPlayback = prefs[BACKGROUND_PLAYBACK] ?: false,
                    floatingWindow = prefs[FLOATING_WINDOW] ?: false
                )
            } catch (e: Exception) {
    ...
    suspend fun updateBackgroundPlayback(enabled: Boolean) {
        try {
            context.dataStore.edit { prefs ->
                ...
                // Disable floating window if background playback is enabled
                if (enabled) {
                    prefs[FLOATING_WINDOW] = false
                }
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error saving background playback setting", e)
        }
    }
    ...
    suspend fun updateFloatingWindow(enabled: Boolean) {
        try {
            context.dataStore.edit { prefs ->
                prefs[FLOATING_WINDOW] = enabled
                // Disable background playback if floating window is enabled
                if (enabled) {
                    prefs[BACKGROUND_PLAYBACK] = false
                }
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error saving floating window setting", e)
        }
    }
}
